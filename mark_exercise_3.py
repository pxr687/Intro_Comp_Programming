...
def check_answer_1(total, 
    ...
    conditions = np.array([soft_assert(type(total) != str,
                              f"Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should be either `int` or `float` data type! `{answer_name}` is the `str` datatype at the moment. You will see a NASTY error below this message, because of this!",
...
def check_answer_3(test_score, 
    ...
    conditions = np.array([soft_assert(type(test_score) != str,
                              f"Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should be the `float` data type! `{answer_name}` is the `str` datatype at the moment. You will see a NASTY error below this message, because of this!",
                                return_mark=return_mark),
...
def check_answer_4(welcome_20, 
    ...
    conditions = np.array([soft_assert(welcome_20 == mk_t_3()[3].astype(str),
                              f"Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` is the wrong value! It should be the string: `{mk_t_3()[3]}`",
                                return_mark=return_mark)])
...
def check_answer_5(n_forms, 
    ...
    conditions = np.array([soft_assert(type(n_forms) != str,
                              f"Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should be `float` or `int` data type! `{answer_name}` is the `str` datatype at the moment. You will see a NASTY error below this message, because of this!",
                                return_mark=return_mark),
