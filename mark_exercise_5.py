...
def check_answer_1(pop_thailand, 
    ...
    conditions = np.array([soft_assert(np.isclose(pop_thailand, 71_600_000),
                              f"Your answer to Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should equal 71,600,000!",
...
def check_answer_2(are_they_the_same, 
    ...
    conditions = np.array([soft_assert(type(are_they_the_same) == bool,
                           f"Your answer to Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should be a `bool`! Right now it is `{type(are_they_the_same)}`.",
                                                                       return_mark=return_mark),
                            soft_assert(are_they_the_same == (71600000 == 71_600_000),
                            f"Your answer to Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` is the wrong value!",
                                return_mark=return_mark)])
...
def check_answer_3(answer_3, 
    ...
    conditions = np.array([soft_assert(answer_3 == int,
                              f"Your answer to Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should tell us the `type` of  the variable `forty_two`! Right now `{answer_name} == {answer_3}`.",
                                return_mark=return_mark)])
...
def check_answer_4(forty_two_as_float,
    ...
    conditions = np.array([soft_assert(type(forty_two_as_float) == float,
                              f"Your answer to Your answer to {question_name} is **NOT CORRECT!** `{answer_name}` should be a float! Right now it is `{type(forty_two_as_float)}`.",
                                return_mark=return_mark),
